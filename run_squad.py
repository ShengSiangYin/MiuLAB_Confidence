...
def ensemble(args, all_results, all_logits, unique_id2example_index, examples):
    ...
    for i in range(len(all_results[0])):
        ...
        logit = np.zeros(args.ensemble)
        for j in range(args.ensemble):
            logit[j] = all_logits[j][examples[unique_id2example_index[all_results[j][i].unique_id]].qas_id]['cls_diff']
        unique_id = all_results[0][i].unique_id
        start_logits = np.zeros(384)
        end_logits = np.zeros(384)
        ...
        for j in range(args.ensemble_top_n):
            ...
            end_logits += ratio[j] * np.array(all_results[top_indices[j]][i].end_logits)

        results.append(SquadResult(unique_id, list(start_logits), list(end_logits)))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for model_idx in range(args.ensemble+1):
        print(f"Evaluate Model No.{model_idx}")
        ...
        if args.model_type in ["xlnet", "xlm"]:
            ...
            predictions, logits_json = compute_predictions_logits(
                ...
                tokenizer,
                ensemble=True if model_idx == args.ensemble else False,
            )
